...
func dataHandler(w http.ResponseWriter, r *http.Request){
	...
	var res string;
	_,ok:=hash[username]
	if !ok  || hash[username].PasswordHash!=passHash||time.Now().Sub(hash[username].LastTime ) > time.Second*60*3 {
		chanel := make(chan string)
		go post(username,password,chanel)
		res = <-chanel
		go func(){
			hash[username] = CacheData {LastTime:time.Now(),Data:res,PasswordHash:passHash}
		}()
